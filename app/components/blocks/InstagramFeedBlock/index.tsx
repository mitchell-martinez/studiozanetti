...
import styles from './InstagramFeedBlock.module.scss'
import type { InstagramFeedBlockProps } from './types'

const InstagramFeedBlock = ({ block }: InstagramFeedBlockProps) => {
      ...
      <div className={styles.inner}>
        ...
        >
          {block.images.map((image, index) => (
            <a
              key={`${image.url}-${index}`}
              href={block.profile_url}
              target="_blank"
              rel="noopener noreferrer"
              className={styles.imageItem}
              aria-label={image.alt || `Instagram post ${index + 1}`}
            >
              <img
                src={image.url}
                alt={image.alt || ''}
        ...
        <div className={styles.cta}>
          <Button
            ...
            variant="outline"
            openInNewTab
            ariaLabel={`Follow @${block.username} on Instagram`}
          >
            <svg
            ...
            {block.cta_text || ` Follow @${block.username}`}
          </Button>
        </div>
      </div>
    </section>
