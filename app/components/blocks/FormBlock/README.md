...
- Enable it on exactly one Email field when `Offer Submitter Email Copy` is on.
...
- When `Offer Submitter Email Copy` is enabled, exactly one Email field must also enable `Use For Submitter Copy`.
